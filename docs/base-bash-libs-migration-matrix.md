...
| `repo_installer_template.sh` (`repo installer-template`) | At most one installer path positional; `--repo`, `--issue`, and `--category` take values; `--print`/`--stdout`, `--pr`, `--dry-run`, and `-v` are flags. No forwarded-argument boundary; the pre-parser preserves legacy help, missing-value, unknown-option, and equals-form behavior. | Duplicate scalar values keep the last value; `--print` conflicts with `--pr` and with an explicit path as before. Generated template content, executable-file writes, and optional PR/worktree boundaries remain local. | `cli/bash/commands/basectl/tests/repo.bats` installer-template cases | Migrated in the #2144 leaf; preserve stdout/path safety and generated installer PR behavior. |
...
| `gh_branch_worktree.sh` (`worktree prune`) | No positionals; `--dry-run`, `--yes`, and `--closed-unmerged`. No forwarded-argument boundary; `--remote` remains rejected because remote cleanup belongs to branch prune. | Help, unknown-option, and remote-cleanup errors remain immediate. Duplicate flags remain accepted; `--dry-run` and `--yes` remain mutually exclusive; dry-run is the default. Worktree eligibility, GitHub verification, output, and deletion status remain local to the pruning layer. | `cli/bash/commands/basectl/tests/gh-branch-worktree.bats` worktree prune cases | Migrated in the #2144 leaf; preserve default dry-run and local-only cleanup boundaries. |
...
reopen all parser migrations. The nested parser leaves identified for
#2122/#2144 are now migrated. Remaining `while` loops are intentional
compatibility normalizers, pass-through scanners, or stateful top-level
dispatch/JSON processing; they must not be converted without a new behavior
baseline.
