...
| `basectl workspace test` | Run declared project test commands serially from each selected repository, without rediscovering unselected siblings. Selection prefers a repository name, then an unambiguous project name; selecting two aliases of the same manifest is rejected. Projects without a declared test command are skipped; failures are reported per project and `--fail-fast` stops after the first failure. | `--workspace <path>`, `--manifest <path>`, `--projects <name[,name...]>`, `--fail-fast`, `--format <text\|json>` |
