...
#     - keep Zsh dotfile snippets from duplicating this logic
#
...
base_zsh_baserc_guard_debug() {
    ...
    whence -w "$debug_function" >/dev/null 2>&1 || return 0
    "$debug_function" "$*"
}

base_zsh_baserc_guard_owned_vars() {
    printf '%s\n' "BASE_HOME BASE_BIN_DIR BASE_CLI_DIR BASE_BASH_DIR BASE_BASH_COMMANDS_DIR BASE_LIB_DIR BASE_BASH_LIB_DIR BASE_SHELL_DIR BASE_OS BASE_HOST BASE_SHELL BASE_PROFILE_VERSION BASE_ENABLE_BASH_DEFAULTS BASE_ENABLE_ZSH_DEFAULTS"
}

base_zsh_baserc_guard_reject_owned_var() {
    local var_name="$1"
    local before_set="$2"
...
base_zsh_baserc_guard_source() {
    ...
    for var_name in ${=base_owned_vars}; do
        snapshot_name="base_zsh_baserc_guard_before_$var_name"
    ...
    for var_name in ${=base_owned_vars}; do
        ...
        base_zsh_baserc_guard_reject_owned_var "$var_name" "$before_set" "$before_value" || baserc_status=1
    done
    ...
    base_zsh_baserc_guard_debug "$debug_function" "sourced '$baserc'"
}
